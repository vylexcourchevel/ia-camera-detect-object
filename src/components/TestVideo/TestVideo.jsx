...
function TestVideo() {
  ...
  const startup = () => {
    ...
    navigator.mediaDevices
      .getUserMedia({ video: true, audio: false }) // Elle renvoi un StreamVideo
      .then(function (stream) {
        // Avec le .then on attend que la promesse soit terminée
    ...
    video.addEventListener(
      ...
      (event) => {
        if (!isStreaming) {
          ...
          if (isNaN(height)) {
            // Si la hauteure n'est pas un nombre
  ...
  const takepicture = () => {
    ...
    if (width && height) {
      // Si la largeur et la hauteur existent, on règle celle du canvas
      canvas.width = width;
